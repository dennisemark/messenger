...
let sendStatus = (action,recipient) => {
    request({
        url: 'https://graph.facebook.com/v2.6/me/messages',
        qs: {access_token: process.env.FB_PAGE_TOKEN},
        method: 'POST',
...
let processText = (text, sender)  => {
	...
	sendMessage({text: 'No result found!'}, sender);
	
};


//This code adds support for the Messenger Platform's webhook verification to our webhook. 
//This is required to ensure our webhook is authentic and working.
let handleGet = (req, res) => {
    if (req.query['hub.verify_token'] === process.env.FB_VERIFY_TOKEN) {
		//FB_VERIFY_TOKEN is a random string defined by the developer
        res.send(req.query['hub.challenge']); // Responds with the challenge token from the request
    }
    res.send('Error, wrong validation token');
};

let handlePost = (req, res) => {
    let events = req.body.entry[0].messaging; // Gets the message. entry[0].messaging is an array containing all the messages
    ...
    res.sendStatus(200); // Returns a '200 OK' response to all requests
};
